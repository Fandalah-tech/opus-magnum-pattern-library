(() => {
  ...
  function renderFacts(graph) {
    ...
    const items = [
      [t("inspector.nodes"), summary.nodeCount],
      [t("nav.relations"), summary.edgeCount],
      [t("inspector.components"), summary.componentCount],
      [t("inspector.arms"), summary.armCount],
    ];
  ...
  function renderParts(solution) {
    ...
    byId("part-summary").innerHTML = [...counts.entries()]
      ...
      .map(([type, count]) => `<span>${type} × ${count}</span>`).join("") || `<span>${t("inspector.noParts")}</span>`;
  ...
  function renderTimeline(timeline) {
    ...
    const facts = [
      [t("inspector.activeCycles"), summary.activeCycleCount],
      [t("inspector.globalIdle"), summary.globalIdleCycles],
      [t("inspector.peakParallel"), summary.peakParallelArms],
      [t("inspector.averageParallel"), summary.averageParallelArms],
    ];
  ...
  function renderRelations(graph) {
    ...
    ).join("") || `<span>${t("inspector.noRelations")}</span>`;
  }
