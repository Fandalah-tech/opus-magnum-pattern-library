(() => {
  ...
  function segment(layer, a, b) {
    ...
    layer.appendChild(svgEl('line', {x1:a.x,y1:a.y+2.5,x2:b.x,y2:b.y+2.5,stroke:'#020302','stroke-width':20,'stroke-linecap':'round',opacity:.82}));
    layer.appendChild(svgEl('line', {x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:'#17120d','stroke-width':17,'stroke-linecap':'round'}));
    layer.appendChild(svgEl('line', {x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:'#050706','stroke-width':11.2,'stroke-linecap':'round'}));
    [-railOffset, railOffset].forEach(offset => {
      const x1 = a.x + nx * offset, y1 = a.y + ny * offset, x2 = b.x + nx * offset, y2 = b.y + ny * offset;
      layer.appendChild(svgEl('line', {x1,y1,x2,y2,stroke:'#3b2a18','stroke-width':4.3,'stroke-linecap':'round'}));
      layer.appendChild(svgEl('line', {x1,y1:y1-.45,x2,y2:y2-.45,stroke:'#a47b42','stroke-width':2.55,'stroke-linecap':'round'}));
      layer.appendChild(svgEl('line', {x1,y1:y1-1,x2,y2:y2-1,stroke:'#d7b06d','stroke-width':.65,'stroke-linecap':'round',opacity:.72}));
    });
  }
  ...
  function textMarker(layer, x, y, label) {
    const text = svgEl('text', {x,y,'text-anchor':'middle','dominant-baseline':'middle','font-family':'Georgia, Times New Roman, serif','font-size':18,'font-weight':700,fill:'#e8d6a5',stroke:'#17120d','stroke-width':1.1,'paint-order':'stroke','data-track-marker':label});
    text.textContent = label;
    layer.appendChild(text);
  }

  function endpointMarker(layer, point, neighbor, label) {
    const dx = neighbor.x - point.x, dy = neighbor.y - point.y, length = Math.hypot(dx, dy) || 1;
    textMarker(layer, point.x + (-dy / length) * 17, point.y + (dx / length) * 17, label);
  }

  function loopMarkerPair(layer, point, next, previous) {
    const dx = next.x - previous.x, dy = next.y - previous.y, length = Math.hypot(dx, dy) || 1;
    const nx = -dy / length, ny = dx / length;
    textMarker(layer, point.x + nx * 17, point.y + ny * 17, '+');
    textMarker(layer, point.x - nx * 17, point.y - ny * 17, '−');
  }
  ...
  function renderTrack(layer, track, board) {
    ...
    for (let index = 0; index < segmentCount; index++) segment(layer, uniquePoints[index], uniquePoints[(index + 1) % uniquePoints.length]);
    uniquePoints.forEach(point => joint(layer, point));
    if (loop) {
      const controlIndex = Math.max(0, Math.min(uniquePoints.length - 1, Number.isInteger(track.controlIndex) ? track.controlIndex : 0));
      const point = uniquePoints[controlIndex], next = uniquePoints[(controlIndex + 1) % uniquePoints.length], previous = uniquePoints[(controlIndex - 1 + uniquePoints.length) % uniquePoints.length];
      loopMarkerPair(layer, point, next, previous);
      return;
    }
    endpointMarker(layer, uniquePoints[0], uniquePoints[1] || uniquePoints[0], '−');
    const last = uniquePoints.length - 1;
    endpointMarker(layer, uniquePoints[last], uniquePoints[last - 1] || uniquePoints[last], '+');
  }
