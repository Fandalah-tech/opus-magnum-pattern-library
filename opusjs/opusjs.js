(() => {
  ...
  function drawAtom(svg,item,scene,id){
    ...
    const c=elements[item.element]||elements.Fe;
    const g=el('g',{transform:`translate(${p.x} ${p.y})`,filter:`url(#shadow-${id})`});
    g.appendChild(el('circle',{r:27,fill:'#090c0b',stroke:'#121817','stroke-width':2}));
    g.appendChild(el('circle',{r:24.5,fill:`url(#bevel-${id})`,stroke:'#3b372d','stroke-width':1.4}));
    g.appendChild(el('circle',{r:20.5,fill:c.dark,stroke:'#151815','stroke-width':1.5}));
    g.appendChild(el('circle',{r:18.2,fill:`url(#atom-${item.element}-${id})`,stroke:c.light,'stroke-width':1.3}));
    g.appendChild(el('circle',{r:14.6,fill:'none',stroke:c.dark,'stroke-width':1.5,opacity:.85}));
    g.appendChild(el('ellipse',{cx:-5,cy:-7,rx:8.2,ry:4.2,fill:'#fff',opacity:.18,transform:'rotate(-20)'}));
    const t=el('text',{x:0,y:6,'text-anchor':'middle','font-family':'Georgia,Times New Roman,serif','font-size':17,'font-weight':700,fill:c.ink,stroke:c.light,'stroke-width':.35,'paint-order':'stroke'});t.textContent=item.element;g.appendChild(t);
    for(let i=0;i<6;i++){const a=i*Math.PI/3;g.appendChild(el('circle',{cx:23.2*Math.cos(a),cy:23.2*Math.sin(a),r:1.5,fill:'#2e2b23',stroke:'#c8bb93','stroke-width':.7}));}
    svg.appendChild(g);
  }
  ...
  window.OpusJS={version:'0.2.0',render,axial};
})();