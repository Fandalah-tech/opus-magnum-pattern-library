(() => {
  ...
  const atomGeometry={holderRadius:28.0,outerCasingRadius:24.0,bevelRadius:22.65,innerCasingRadius:20.15,coreRadius:18.7,innerRingRadius:16.05,rivetOrbit:21.15,rivetRadius:.42};
  const elements={
    Pb:{base:'#46585a',mid:'#6f7f7c',dark:'#202d2e',light:'#c2ccca',ink:'#f5f5ef',mark:'♄',symbolSize:21.2,symbolY:0},
    Sn:{base:'#996844',mid:'#bd8a62',dark:'#573721',light:'#dfb68e',ink:'#f4e7d8',mark:'♃',symbolSize:18.2,symbolY:-.1},
    Fe:{base:'#777e83',mid:'#a3a9ad',dark:'#3d4246',light:'#d1d5d6',ink:'#f0f0ec',mark:'♂',symbolSize:18.2,symbolY:-.1},
    Cu:{base:'#9b5934',mid:'#c77b4b',dark:'#5a301e',light:'#e5a574',ink:'#f7e8dd',mark:'♀',symbolSize:18.2,symbolY:-.1},
    Ag:{base:'#a7aaa3',mid:'#d4d5cf',dark:'#60645f',light:'#f0efe8',ink:'#ffffff',mark:'☽',symbolSize:18.2,symbolY:-.1},
    Au:{base:'#a98425',mid:'#d4ad42',dark:'#5f4816',light:'#f1d77c',ink:'#fff4c2',mark:'☉',symbolSize:18.2,symbolY:-.1},
    Hg:{base:'#8699a1',mid:'#b8c7cc',dark:'#4d6068',light:'#dce8eb',ink:'#f5fbfc',mark:'☿',symbolSize:18.2,symbolY:-.1}
  };
  ...
  function defs(svg,id){
    ...
    Object.entries(elements).forEach(([symbol,c])=>{
      const radial=el('radialGradient',{id:`atom-${symbol}-${id}`,cx:'43%',cy:symbol==='Pb'?'34%':'24%',r:symbol==='Pb'?'82%':'78%'});
      [['0%',c.light],['24%',c.mid],['64%',c.base],['100%',c.dark]].forEach(([offset,color])=>radial.appendChild(el('stop',{offset,'stop-color':color})));d.appendChild(radial);
    });
    svg.appendChild(d);
  }
  ...
  function drawAtom(svg,item,scene,id){
    ...
    const m=atomGeometry;
    const g=el('g',{transform:`translate(${p.x} ${p.y})`,'data-atom':element});
    const body=el('g',{filter:`url(#shadow-${id})`});
    body.appendChild(el('polygon',{points:hexPoints(0,0,m.holderRadius),fill:`url(#holder-${id})`,stroke:'#806047','stroke-width':.72}));
    body.appendChild(el('polygon',{points:hexPoints(0,0,m.holderRadius-1.7),fill:'#171411',stroke:'#29211c','stroke-width':.62}));
    body.appendChild(el('circle',{r:m.outerCasingRadius,fill:'#090908',stroke:'#201a17','stroke-width':.96}));
    body.appendChild(el('circle',{r:m.bevelRadius,fill:`url(#bevel-${id})`,stroke:'#2f241d','stroke-width':.7}));
    body.appendChild(el('circle',{r:m.innerCasingRadius,fill:'#111513',stroke:'#070909','stroke-width':.76}));
    body.appendChild(el('circle',{r:m.coreRadius,fill:`url(#atom-${element}-${id})`,stroke:c.light,'stroke-width':.5}));
    body.appendChild(el('circle',{r:m.innerRingRadius,fill:'none',stroke:c.dark,'stroke-width':.72,opacity:.58}));
    ...
    for(let i=0;i<6;i++){const a=i*Math.PI/3;body.appendChild(el('circle',{cx:m.rivetOrbit*Math.cos(a),cy:m.rivetOrbit*Math.sin(a),r:m.rivetRadius,fill:'#3a3027',stroke:'#b99d70','stroke-width':.2,opacity:.46}));}
    g.appendChild(body);
    const t=el('text',{x:0,y:c.symbolY,'text-anchor':'middle','dominant-baseline':'middle','font-family':'Noto Serif Symbols 2, Segoe UI Symbol, DejaVu Sans, serif','font-size':c.symbolSize,'font-weight':300,fill:c.ink,'text-rendering':'geometricPrecision','pointer-events':'none',opacity:element==='Pb'?.95:.96});
  ...
  window.OpusJS={version:'0.6.0',render,axial,atomGeometry};
})();