...
from packages.opus_solver.trajectory_glyph_repair import search_trajectory_guided_purification
...
def probe(
    ...
    *,
    ...
    search = search_trajectory_guided_purification(
        puzzle,
        base_solution,
    ...
    for index, variant in enumerate(search.get("variants") or []):
        ...
        compact_variants.append({
            ...
            "trajectoryEvidence": variant.get("trajectoryEvidence"),
    ...
    return {
        "schemaVersion": "0.1.0",
        "kind": "strict-heldout-trajectory-purification-probe",
        ...
        "request": {
            ...
            "poseLimit": pose_limit,
            "resultLimit": result_limit,
        },
        "searchSummary": search.get("summary"),
        "discoveredPoses": search.get("poses"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Relocate purification glyphs using only heldout-candidate trajectories.")
    ...
    parser.add_argument("--discovery-cycles", type=int, default=132)
    parser.add_argument("--validation-cycles", type=int, default=320)
    parser.add_argument("--pose-limit", type=int, default=96)
