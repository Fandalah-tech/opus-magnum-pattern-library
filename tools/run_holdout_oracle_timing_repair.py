    ...
    for part_index, part in enumerate(solution.get("parts", []) or []):
        ...
        for delta in range(-3, max(0, int(max_delay)) + 1):
            ...
            variants.append({
                ...
                "armPartId": str(part.get("id") or f"part-{part_index}"),
                "delta": delta,
                "solution": candidate,
            })

        # Shift one nearby instruction while preserving unique tape cells. This
        # can delay a grab without unnecessarily delaying a later reaction pose.
...
def search(
    *,
    ...
    if collision_cycle is None:
        return {
            "schemaVersion": "0.1.0", "kind": "strict-heldout-oracle-timing-repair-search",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Repair a blind OMSim frontier by perturbing only instructions near the collision cycle.")
    ...
    print(json.dumps({
        "baselineOMSim": report["baselineOMSim"], "searchedVariantCount": report["searchedVariantCount"],
