...
from tools.evaluate_holdout_transfer import evaluate_holdout_transfer
...
def evaluate_matrix(manifest_path: Path, puzzle_root: Path, flow_index_path: Path, output_dir: Path, *, objective: str = "balanced", composition_limit: int = 30) -> dict[str, Any]:
    ...
    for target in manifest["targets"]:
        ...
        report = evaluate_holdout_transfer(puzzle_path, flow_index_path, target_puzzle_id=target_id, objective=objective, composition_limit=composition_limit, solution_output=solution_output)
    ...
    return {
        ...
        "request": {"objective": objective, "compositionLimit": int(composition_limit), "targetSolutionBytesUsed": 0, "targetSolutionInputsAccepted": False, "directGeneratorAllowed": False, "learnedArchitectureBankAllowed": False, "fragmentKnowledgeAllowed": True},
        "summary": {
