...
USER_AGENT = "OpusMagnumCodexResearch/0.4 (+https://github.com/Fandalah-tech/opus-magnum-pattern-library)"
...
def discover_submissions_url(links: Iterable[Link], origin: str) -> str | None:
    candidates: list[str] = []
    for link in links:
        if not same_origin(link.url, origin) or link.url.lower().startswith("data:"):
            continue
        path = urlparse(link.url).path
        if re.fullmatch(r"/submissions/[0-9a-f]{16,}", path, re.I):
...
def write_binary(root: Path, category: str, link: Link, data: bytes, suffix: str) -> dict:
    ...
    return {"url": public_source_url(link), "downloadName": link.download_name, "path": path.as_posix(), "sha256": digest, "bytes": len(data)}
...
def main() -> int:
    ...
    parser.add_argument("--submissions-url", default="", help="Optional. If omitted, discover /submissions/<id> from the event page.")
    ...
    submissions_url = args.submissions_url or discover_submissions_url(event_links, args.event_url)
    if not submissions_url:
        raise SystemExit("No public submissions page discovered on event page; pass --submissions-url explicitly if needed.")

    time.sleep(max(0.0, args.delay))
    submissions_html, submissions_type = fetch(submissions_url)
    submission_links, submission_forms, submission_embedded = parse_links(submissions_html, submissions_url)
    solution_links = discover_file_links(submission_links, submission_embedded, ".solution", submissions_url)
    ...
    report: dict = {
        ...
        "event": {"url": args.event_url, "contentType": event_type, "forms": event_forms, "puzzleLinks": [public_link(link) for link in puzzle_links]},
        "submissions": {"url": submissions_url, "contentType": submissions_type, "forms": submission_forms, "solutionLinks": [public_link(link) for link in solution_links]},
        ...
        "policy": {
            ...
            "submissionsPageAutoDiscovery": True,
    ...
    summary = {"puzzleLinks": len(puzzle_links), "solutionLinks": len(solution_links), "downloadedPuzzles": len(report["downloaded"]["puzzles"]), "downloadedSolutions": len(report["downloaded"]["solutions"]), "puzzleFailures": len(report["failures"]["puzzles"]), "solutionFailures": len(report["failures"]["solutions"]), "submissionsUrl": submissions_url, "report": args.report.as_posix()}
    print(json.dumps(summary, ensure_ascii=False))
    return 0 if args.discover_only or (report["downloaded"]["puzzles"] and report["downloaded"]["solutions"]) else 3
