...
def validate_portfolio_materialization(
    ...
    *,
    ...
    for variant_index, objective in enumerate(("cga", "bca")):
        ...
        output_file = None
        if output_dir is not None:
            output_file = output_dir / f"aqueous-dagger-{objective}-learned.solution"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rebuild learned Critelli winner blueprints and validate them without original solution bytes.")
