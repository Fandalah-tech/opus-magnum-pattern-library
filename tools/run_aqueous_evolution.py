...
def main() -> int:
    ...
    elites = [{"path": seed_copy, "metrics": seed_metrics, "kind": "seed", "generation": 0}]

    hall: dict[str, dict] = {
        solution_id(seed_copy): {**elites[0], "sha256": solution_id(seed_copy)}
    }
    cumulative_tested = 0
    cumulative_valid = 0
    ...
    best_generation = 0
    stagnant = 0
    history = [{"generation": 0, "bestCycles": best_cycles, "tested": 0, "valid": 1}]
    ...
    def top3() -> list[dict]:
        ranked = sorted(hall.values(), key=lambda x: (score(x.get("metrics") or {}), x.get("sha256", "")))[:3]
        result = []
    ...
    for gen in range(1, generations + 1):
        ...
        print(f"GENERATION {gen}/{generations} candidates={generation_total} parents={len(elites)}", flush=True)
        publish("running")
            ...
            for future in as_completed(futures):
                ...
                if validation.get("valid") is True:
                    ...
                    if cycles < best_cycles:
                        ...
                        best_generation = gen
                        stagnant = 0
                        print("NEW GLOBAL BEST", json.dumps({"generation": gen, "metrics": metrics, "kind": record["kind"]}, sort_keys=True), flush=True)
                ...
                if generation_tested % max(1, args.progress_every) == 0 or generation_tested == generation_total:
                    print(
                        f"PROGRESS generation={gen}/{generations} tested={generation_tested}/{generation_total} cumulative={cumulative_tested} valid={generation_valid} best={best_cycles}",
                        flush=True,
                    )
                    publish("target_reached" if best_cycles <= args.target_cycles else "running")

        previous_best = history[-1]["bestCycles"]
        ranked_all = sorted(hall.values(), key=lambda x: (score(x.get("metrics") or {}), x.get("sha256", "")))
        elites = []
    ...
    ranked_final = sorted(hall.values(), key=lambda x: (score(x.get("metrics") or {}), x.get("sha256", "")))[:10]
    report = {
        "state": state,
