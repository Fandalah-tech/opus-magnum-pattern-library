...
def test_singleton_input_is_recentered_on_first_stationary_grab():
    ...
    assert aligned["originalPosition"] == [3, -2]
    assert aligned["grabPosition"] == [2, -1]
    assert aligned["position"] == [2, -1]
    assert aligned["servingArmId"] == "arm"
    assert aligned["exactFragmentInstanceMatch"] is True


def test_local_singleton_atom_offset_is_respected_under_input_rotation():
...
def test_multi_atom_reagent_is_never_recentered():
    puzzle = _puzzle([
    ...
    assert singleton_input_alignment(input_part, 0, puzzle, [input_part, arm]) is None


def test_unrelated_arm_cannot_attract_singleton_input():
    puzzle = _puzzle([{"id": "a0", "element": "earth", "position": [0, 0]}])
    input_part = {
        "id": "input",
        "type": "input",
    ...
    arm = {
        ...
        "rotation": 3,
        "length": 1,
        "sourceFragmentInstances": ["branch-9-other"],
        "program": [{"cycle": 0, "instruction": "grab"}],
    }
...
def test_arm_with_strongest_shared_fragment_provenance_wins():
    puzzle = _puzzle([{"id": "a0", "element": "earth", "position": [0, 0]}])
    input_part = {
        "id": "input",
        "type": "input",
    ...
    aligned = singleton_input_alignment(
        input_part,
        0,
        puzzle,
        [input_part, partial_arm, exact_arm],
    )

    assert aligned is not None
    assert aligned["servingArmId"] == "exact"
    assert aligned["sharedFragmentInstanceCount"] == 2
    assert aligned["exactFragmentInstanceMatch"] is True
