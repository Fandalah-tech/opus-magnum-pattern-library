...
class InitialOverlapRepairTests(unittest.TestCase):
    def test_relocates_arm_base_while_preserving_tip(self) -> None:
        ...
        self.assertEqual(repair["oldLength"], 1)
        self.assertEqual(repair["newLength"], 1)
        self.assertEqual(repair["targetSolutionBytesUsed"], 0)

    def test_can_lengthen_arm_while_preserving_tip(self) -> None:
        solution = {
            "source": {},
            "parts": [{
                "id": "cleanup",
        ...
        arm = updated["parts"][0]
        self.assertEqual(arm["position"], [7, 1])
        self.assertEqual(arm["rotation"], 0)
        self.assertEqual(arm["length"], 2)
        repair = updated["source"]["initialArmBaseRepairs"][0]
        self.assertEqual(repair["preservedTip"], [9, 1])
        self.assertEqual(repair["oldLength"], 1)
        self.assertEqual(repair["newLength"], 2)
        self.assertEqual(repair["targetSolutionBytesUsed"], 0)
